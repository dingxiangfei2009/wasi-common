...
let
  ...
  resolver = { source, name, version, ... }: {
    unknown.wasi-common."0.1.0" = pkgs.rustBuilder.rustLib.cleanLocalSource srcFilter ./.;
    unknown.wasi-common-cbindgen."0.1.0" = pkgs.rustBuilder.rustLib.cleanLocalSource srcFilter ./wasi-common-cbindgen;
  }.${source}.${name}.${version};
  ...
  rustPackages = pkgs.callPackage ./crate.nix {
    ...
    target.isWasi = false;
    target.isUnix = false;
    target.isFreeBSD = false;
    target.isWindows = false;
    target.isMacOS = false;
    target.isiOS = false;
    target.isLinux = true;
    # TODO: this is very confusing
    target.system = "wasm32-emscripten";
    target.cpu.name = "wasm32";
    target.abi.name = "emscripten";
    target.kernel.name = "unknown";
    target.targetPrefix = "em";
    target.nativeBuildInputs = [ pkgs.buildPackages.emscripten ];
  };
...
in
{
  ...
  package = rustPackages.unknown.wasi-common."0.1.0";
